# Neon Preview Environments Skill For Claude Code

This Claude Code skill helps install an issue-driven preview environment workflow into an application repository:

```text
GitHub issue -> trusted @claude trigger -> Claude branch -> pull request -> Neon preview branch -> live preview URL -> cleanup on merge/close
```

It is a template and adaptation skill. It gives Claude provider-specific workflow templates, scripts, and references, but the installing agent must still inspect the target repo and replace placeholders with that user's real project details.

## Install

Install it as either a personal or project Claude Code skill.

Personal install:

```bash
mkdir -p ~/.claude/skills
cp -R ai/skills/claude-code/preview-environments/neon ~/.claude/skills/neon-preview-environments
```
...
Claude Code watches existing skill directories for changes. Restart Claude Code if the top-level skills directory did not exist when the session started.

## Use

Open Claude Code in the target application repository and invoke the skill explicitly:

```text
/neon-preview-environments

Install an issue-driven Claude Code preview workflow in this repo.
...
Claude runtime secrets, choose one:

- `CLAUDE_CODE_OAUTH_TOKEN`: long-lived Claude subscription token from `claude setup-token`.
...
- `agent-claude.yml`: trusted issue/comment trigger, full issue context collection, Claude Code implementation, checks, PR creation.
...
- Do not print database URLs, Claude OAuth/API tokens, provider tokens, or registry credentials.
...
```bash
python3 ai/skills/claude-code/preview-environments/neon/scripts/validate_skill.py
```
