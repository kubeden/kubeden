...
function triggerMatches(body) {
  if (provider === "codex") {
    return /(^|\s)(@codex\b|@agent\b|@preview-agent\b|\/agent\s+codex\b|\/preview-agent\b)/i.test(body);
  }
...
const prTitle = pull ? pull.title : `${provider === "codex" ? "Codex" : "Claude"}: ${issue.title}`;
